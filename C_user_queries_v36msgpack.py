...
def convert_to_select(query):
    ...
    s_query = [
        'SELECT ?o3 ?i5 ?n ?t1 ?t3',
        'where {',
        '<http://example.org/subjects/s1> <http://xmlns.com/foaf/0.1/knows> <http://example.org/objects/o1000013> ?i5[,,(?t1,?t3)] ?i6 .',
        '?i5 <http://purl.org/biotop/biotop.owl#derivesFrom> ?o3 ?i7[,,(,)] ?i4 .',
        '}'
    ]
    """
...
def write_csv_file(filename, result):
    ...
    writer1 = writer(open(filename, "w"))
...
def sort_queries(query, data):
    ...
    while i <= len(query) - 2:
        ...
        if not line[1].startswith('?'):
            ...
            if p1 not in data:
                print('Loading data for', p1)
                data = get_predicate_val(data, p1)
...
def uncompress_result(result, rindexes):
    ...
    if rindexes == dict():
        ...
        with open(rindex_file, "rb") as data_file:
            byte_data = data_file.read()
        rindexes = unpackb(byte_data)
        '''with open(rindex_file) as f:
            rindexes = json.load(f)'''
        print('Loaded.')
        logging.info('Loaded.')
